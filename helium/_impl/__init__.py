...
from helium._impl.util.system import get_canonical_os_name, is_windows
...
class SImpl(HTMLElementImpl):
	...
	def find_all_in_curr_frame(self):
		def wrap(web_elements): return list(map(WebElementWrapper, web_elements))
...
class HTMLElementIdentifiedByXPath(HTMLElementImpl):
	...
	def _sort_search_result(self, search_result):
		...
		def sort_key(tpl): return tpl[0]
		keys_to_result_items.sort(key=sort_key)
		def result_item(tpl): return tpl[1]
