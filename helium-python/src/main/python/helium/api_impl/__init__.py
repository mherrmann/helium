...
class APIImpl(object):
	...
	def start_ie_impl(self, url=None):
		...
		try:
			...
			if 'Protected Mode settings are not the same for all zones.' in msg:
				exc_to_raise = WebDriverException(
			...
			raise exc_to_raise
		atexit.register(self._kill_service, ie_browser.iedriver)
		return self._start(ie_browser, url)
	...
	def _point_to_element_and_offset(self, point):
		...
		if offset == (0, 0) and driver.is_firefox():
			# In some CSS settings (eg. inttest_point.html), the (0, 0) point of
...
class JavaScriptInterval(object):
...
class HTMLElementImpl(GUIElementImpl):
	...
	def _handle_closed_window(self):
		...
		if window_has_been_closed:
			self._driver.switch_to_window(window_handles[0])
